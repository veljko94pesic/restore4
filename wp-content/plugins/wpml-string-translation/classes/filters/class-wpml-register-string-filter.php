...
class WPML_Register_String_Filter extends WPML_Displayed_String_Filter {
	...
	public function translate_by_name_and_context( $untranslated_text, $name, $context = '', &$has_translation = null ) {
		...
		if ( ! $translation && $this->can_register_string( $untranslated_text, $name, $context ) ) {
			...
			if ( ! in_array( $domain, $excluded ) ) {
				...
				$parsed = is_null($settings->get_setting('wpml_st_auto_reg_parsed_contexts')) ? array() : $settings->get_setting('wpml_st_auto_reg_parsed_contexts');
				if ( ! in_array( $domain, $parsed ) ) {
					// Exclude new domains by default
					array_push($excluded, $domain);
					array_push($parsed, $domain);
					$settings->get_settings();
					$settings->update_setting( 'wpml_st_auto_reg_excluded_contexts', $excluded, true );
					$settings->update_setting( 'wpml_st_auto_reg_parsed_contexts', $parsed, true );
				} else {
	...
	private function save_string( $value, $allow_empty_value, $language, $domain, $context, $name ) {
		if ( ! $this->block_save_strings && ( $allow_empty_value || 0 !== strlen( $value ) ) ) {
			...
			$query         = "INSERT INTO {$this->wpdb->prefix}icl_strings ({$query_columns}) VALUES ( {$query_values} )";
