...
class WPML_ST_MO_Dictionary_Storage_Table implements WPML_ST_MO_Dictionary_Storage {
	...
	public function persist() {
		$query = "SELECT file_path_md5 FROM $this->table";
		$db_strings = $this->wpdb->get_col($query);

		// Remove duplicates
		$mo_files = array_map("unserialize", array_unique(array_map("serialize", $this->new_data)));

		// Remove duplicates that exists in database
		$filtered = array_filter($mo_files, function($mo_file) use ($db_strings) {
			return !in_array($mo_file->get_path_hash(), $db_strings);
		});

		foreach ( $filtered as $file ) {
			$sql = "INSERT INTO $this->table ( file_path, file_path_md5, domain, status, num_of_strings, last_modified, component_type, component_id ) VALUES ( %s, %s, %s, %s, %d, %d, %s, %s )";
			$this->wpdb->query(
				$this->wpdb->prepare(
					$sql,
					array(
